...
<div :class="card?'w-25vw py-1':'row w-100 m-0'">
    <template v-if="card">
        <div class="row w-100 m-0 py-2  border-radius-15px " :class="color?'border-azul-2':'border-gray0-1'" @click="click()">
            ...
            <div class="row w-100 m-0">
                <div class="row w-100 m-0 h-60px align-content-center letra-gray3-18 justify-content-center text-center ">{{info.nombre | truncate(26)}}</div>
            </div>
        </div>
    </template>
    <template v-else>
        <div class="row w-100 m-0 py-2" @click="click()">
            <div class="row w-100 m-0 justify-content-center">
                ...
                <div class="col-auto px-0" v-if="selectable" @click="$emit('set', info)" >
                    <div class="row w-100 m-0">
                        <icono icono="check_circle" clase="letra-azul1-30" v-if="check" />
                        <icono icono="radio_button_unchecked" clase="letra-gray2-30" v-else  />
                    </div>
