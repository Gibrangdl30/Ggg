<template>
    <div :class="size ? `${size || ''} py-1` : 'row w-100 m-0'">
        ...
        <modalMensajeStatic
            ...
            boton="Aceptar" v-if="open" @close="open = false" @click="borrar()" />
            ...
            <div class="row w-100 m-0 overflow-hidden" @click="set1()">
                <div class="row w-100 m-0 border-radius-2px ">
                    ...
                    <div class="col my-auto px-0 pl-3">
                        <div class="row w-100 m-0 letra-gray4-16 fw-600">{{ info.name }}</div>
                        <!-- <div class="row w-100 m-0 letra-gray3-16 text-capitalize "> {{ info.cat }}</div> -->
                        <div class="row w-100 m-0  justify-content-start  ">
                            <div class="col-auto px-0">
                                <div class="row w-100 m-0 px-0 my-auto letra-gray2-14 text-capitalize fw-600 "
                                    v-if="info.activate_discount && info.discount > 0">
                                    <s>{{ info.pre_price | currency }} MXN</s>
                                </div>
                                <div
                                    class="row w-100 m-0 px-0 my-auto letra-rojo-14 text-capitalize fw-600 ">
                                    {{ info.price | currency }} MXN</div>
                                <div class="row w-100 m-0 px-0 my-auto letra-rojo1-14 text-capitalize fw-600 "
                                    v-if="info.free_shipping == '1'">Envío gratis</div>
                            </div>
                        </div>
            ...
            <div class="w-45vw" @click="set1()">
                <div class="row w-100 m-0 border-gray0-1 border-radius-10px overflow-hidden">
                    ...
                    <div class="row w-100 m-0 pb-2">
                        <div
                            class="row w-100 m-0 mt-1 h-73px align-content-center overflow-hidden justify-content-center text-center letra-gray4-18 fw-600">
                            {{ info.name | truncate(40) }}</div>
                        <div
                            class="row w-100 m-0 h-55px align-content-center overflow-hidden justify-content-center text-center letra-gray3-16 text-capitalize ">
                            {{ info.cat }}</div>
                        <div class="row w-100 m-0 justify-content-start  ">
                            <div
                                class="row w-100 m-0 h-25px align-content-center justify-content-center text-center px-0 my-auto letra-gray2-16 text-capitalize fw-800 ">
                                <p v-if="info.precioSin" class="m-0"><s>{{ info.precioSin | currency }} MXN</s></p>
                            </div>
                            <div
                                class="row w-100 m-0 justify-content-center text-center px-0 my-auto letra-rojo-19 text-capitalize fw-800 ">
                                {{ info.precio | currency }} MXN</div>
                            <div class="row w-100 m-0 h-25px align-content-center ">
                                <div class="row w-100 m-0 justify-content-center text-center px-0 my-auto letra-rojo1-15 text-capitalize fw-800 "
                                    v-if="info.free_shipping == '1'">Envio gratis</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col px-0 pl-3 my-auto ">
                        ...
                        <div class="row w-100 m-0 pt-1 letra-gray4-17 justify-content-start " v-if="0">
                            <div class="col-auto px-0 pr-2">
                                <div class="row w-100 m-0 px-3 py-1 letra-rojo1-18 border-rojo1-1 border-radius-30px">
                                    Reservar</div>
                            </div>
                            <div class="col-auto px-0 " v-if="0">
                                <div class="row w-100 m-0 px-3 py-1 letra-gray2-18 border-gray2-1 border-radius-30px">Ver
                                    detalles</div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row w-100 m-0 border-gray0-2 border-radius-10px overflow-hidden">
                ...
                <div class="row w-100 m-0 ">
                    ...
                    <div class="col my-auto px-0 pl-3">
                        ...
                        <div class="row w-100 m-0 letra-gray3-16 ">Reservación realizada a las {{ info.created_at | timestamp
                        }} </div>
                        <div class="row w-100 m-0 letra-gray4-20 fw-600">{{ info.campo }}</div>
                        <div class="row w-100 m-0 letra-gray3-18 fw-600">{{ info.lugar }}</div>
                        <div class="row w-100 m-0 letra-gray3-16 text-capitalize " v-if="info.address"> {{ info.address }}
                        </div>
                        <div class="row w-100 m-0 letra-gray3-16 text-capitalize " v-else-if="dire && dire.street">
                            {{ dire.street }} {{ dire.num_ext }}, {{ dire.neighborhood }} </div>
                        ...
                        <div class="row w-100 m-0 letra-gray3-16" v-if="info.status == 'cancelada'">Fecha Cancelacion:
                            {{ info.updated_at | fechaS }} </div>
        ...
        <template v-if="zona">
            <div :class="`row w-100 m-0 border-${show ? 'rojo3' : 'gray3'}-1 border-radius-5px`" @click="show = 1">
                ...
                <div class="row w-100 m-0 px-3 py-2">
                    ...
                    <div class="col my-auto px-0 pl-3">
                        <div class="row w-100 m-0 letra-rojo3-20 fw-600 justify-content-end ">{{ info.precio | currency }}
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col my-auto px-0 pr-2">
                            <div class="row w-100 m-0 px-2 back-color-gray4 border-radius-30px">
                                ...
                                <div class="col px-0 my-auto">
                                    <div class="row w-100 m-0 letra-gray0-25 justify-content-center fw-600 ">{{ cantidad }}
                                    </div>
                                </div>
            ...
            <div :class="`row w-100 m-0 border-${show ? 'rojo3' : 'gray3'}-1 border-radius-5px`" @click="set3()">
                ...
                <div class="row w-100 m-0 px-2 py-2 ">
                    ...
                    <div class="col my-auto px-0 pl-2">
                        <div class="row w-100 m-0 letra-rojo3-18 fw-600 justify-content-end ">Asiento: {{ objCustom.fila ||
                            '-' }}{{ objCustom.asiento ? '-' + objCustom.asiento : '' }}</div>
                    </div>
                </div>
            </div>
        </template>
