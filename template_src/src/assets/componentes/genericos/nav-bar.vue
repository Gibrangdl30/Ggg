...
<div class="nav-bar-component template w-100 pt-15px pt-ios-25px" :class="clase?clase:'pt-10px'" @click="sync">
    ...
    <div class="row m-0 justify-content-space-between px-3px nav-bar nav-inicio  " :class="`${no_border?'':'border-b-gray30-1'}`">
        ...
        <div class="w-13 boton-menu p-0" v-if="backs || emitBack" >
            <template v-if="backs">
                <button type="button" class="button button-outline button-raised button-active-gray3 h-100 p-2px" @click="back()" >
                    <i class="f7-icons color-white letra-gray3-5-5vw">chevron_left</i>
                </button>
            </template>
        ...
        <div class="w-21 boton-menu p-0" v-else-if="customBack" >
            <template >
                <button type="button" class="button button-outline button-raised button-active-gray3 h-100 p-2px" @click="back()">
                    <div class="row w-100 m-0 letra-gray3-4-8vw ">Cancelar</div>
                </button>
            </template>
        </div>
        ...
        <div class="col text-center title p-0">
            ...
            <p class="m-0 letra-gray3-4-8vw my-auto" v-else >{{title}}</p>
        </div>


        <div class="w-12 boton-carrito h-100 position-relative" v-if="chat">
            <button type="button" class="button button-outline button-raised button-active-gray3 h-100 p-2px" @click="router.navigate('/chats')" >
                <i class="material-icons letra-gray3-7-5vw" >chat</i>
            </button>
        ...
        <div class="w-12 boton-carrito h-100"  v-if="right">
            <button type="button" class="button button-outline button-raised button-active-gray3 h-100 p-2px" @click="$emit('right')" v-if="icon">
                <imagen :icono="true" src="scan_gray3" v-if="!icon"/>
    ...
    <div class="row m-0 justify-content-space-between px-3px nav-bar nav-inicio" :class="`${no_border?'':'border-b-gray30-1'}`">
        <div class="w-12 boton-menu p-0" >
            <button type="button" class="button button-outline button-raised button-active-gray3 h-100 p-2px" @click="openMenu()">
                <i class="f7-icons color-white letra-gray3-5-5vw">menu</i>
            </button>
        </div>
        ...
        <div class="w-12 boton-menu p-0" @click="$store.dispatch('synchronizeDataCallback')">
            <button type="button" class="button button-outline button-raised button-active-gray3 h-100 p-2px">
                <i class="material-icons color-white letra-gray3-7-5vw">cached</i>
            </button>
        </div>
    </div>
    </template>
    ...
    <div class="row w-100 m-0 justify-content-space-between px-3px nav-bar nav-inicio border-b-gray30-1">
        ...
        <div class="col text-center title p-0">
            <p class="m-0 letra-gray3-4-1vw my-auto">{{title}}</p>
        </div>
